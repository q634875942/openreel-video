...
**Active Feature:** feat-008 done — Monaco source editor on double-click GeneratedClip (Radix Dialog + lazy chunk + TS intellisense via injected SceneDescription typings). Save → updateGeneratedClipSource → SandboxRegistry.ensure(updated) → awaitReady; compile errors keep dialog open. Folded in: (a) keyboard-shortcuts.ts target whitelist widened (.monaco-editor / [role='dialog'] / isContentEditable) so Space/Ctrl+C/V/Z aren't hijacked into playPause/copyClip/projectUndo; (b) project-store undo+redo dispatch grew 'generated' branch (feat-007 omission — Ctrl+Z couldn't roll back Add-to-timeline); (c) onOpenAutoFocus preventDefault on DialogContent + editor.focus() in onMount so Monaco's keybinding service gets focus and dispatches Ctrl+Z/arrows/Tab/Home/End/shift-select correctly from the first frame. "Ask AI to refactor" deferred to a follow-up feat. Bundle: Monaco chunks (ts.worker 7MB + editor.worker 251kB + SourceEditorDialog 19kB) are lazy in separate files — main bundle delta is +1.77 kB. Next: feat-009 param keyframes (Action/state system).
**Dev server:** stopped
...
- [x] **feat-008 (Slice 4) — Monaco source editor**: Double-click a GeneratedClip on the timeline opens an `apps/web/src/components/SourceEditor/SourceEditorDialog.tsx` (Radix Dialog from @openreel/ui) hosting an @monaco-editor/react Editor with the clip's `source` loaded. Lazy-imported from App.tsx via React.lazy + Suspense, so Monaco's 7MB ts.worker + 251kB editor.worker + 19kB SourceEditorDialog chunk only download when the user actually opens the editor (main bundle grew by +1.77 kB, basically nothing). New files: setupMonacoEnv.ts registers self.MonacoEnvironment.getWorker for the `?worker` imports (only TS + editor workers — json/css/html intentionally omitted), sceneDescriptionTypes.ts hand-mirrors the SceneDescription / Shape / GeneratedObject .d.ts as a string injected via addExtraLib so the user gets intellisense for the AI source contract (with a sync-warning comment placed in SceneDescription.ts pointing here for future-proofing). Save → updateGeneratedClipSource(clipId, draft) → SandboxRegistry.ensure(updated) (must be explicit — otherwise awaitReady races a never-firing canvas-renderers ensure if the playhead is paused outside the clip's range, hitting "clip not in registry") → SandboxRegistry.awaitReady(clipId, 3000). On success closes the dialog; on compile error keeps it open with a red banner displaying the message. SandboxRegistry.ts grew RegistryEntry.initPromise + awaitReady(timeoutMs) that races the init against a setTimeout (+4 unit tests on success / compile-error / unregistered-clip / source-change paths). Reset restores `clip.source`; Cancel discards draft. Dirty-check disables Save when no changes. Three follow-on fixes landed in the same commit because they only manifested while UI-testing this slice: (a) services/keyboard-shortcuts.ts global keydown handler had a too-narrow input-target whitelist (only <input>/<textarea>); widened to also bail on isContentEditable, [role='dialog'], and .monaco-editor — otherwise Space/Ctrl+C/V/Z keys typed in Monaco got hijacked into playPause / copyClip / projectUndo. (b) project-store.ts undo and redo dispatch tables grew a 'generated' branch — feat-007 had omitted this so Ctrl+Z after Add-to-timeline silently no-op'd; undo calls SandboxRegistry.dispose(clipId) before graphicsEngine.deleteGeneratedClip(clipId) to reclaim the Worker, redo re-creates from the entry.clipData snapshot via graphicsEngine.createGenerated and lets the renderer's next frame rebuild the sandbox. (c) Radix DialogContent's onOpenAutoFocus={(e) => e.preventDefault()} stops it from stealing focus to the footer's Reset button; combined with editor.focus() at the end of Monaco's onMount, the editor has focus from the first frame so Ctrl+Z / arrows / Tab / Home/End / shift-select all dispatch correctly inside Monaco. Wire-up: ui-store.ts gained sourceEditorClipId + openSourceEditor + closeSourceEditor; App.tsx mounted <SourceEditorDialog /> via lazy + Suspense; ShapeClipComponent.tsx added handleDoubleClick that only fires for clip.type === 'generated' (greenfield — no prior dblclick handler on clips) and bails when isTrimming/isDragging. Verification: workspace pnpm typecheck clean; pnpm --filter @openreel/web test:run 255 pass / 7 skipped / 1 fail (vs 251 feat-007 baseline = +4 awaitReady tests; same pre-existing project-store.test.ts:851 fail). pnpm --filter @openreel/web build clean in 23.80s with bundle split confirmed (SourceEditorDialog 19.31kB / editor.worker 251kB / ts.worker 7010kB are all lazy chunks). Manual UI smoke confirmed by user: dblclick opens editor; Ctrl+Z / Ctrl+Y / arrows / Tab / Home/End / shift-select+Delete / Space / Ctrl+C/V / typing all behave as native Monaco; Save with valid TS closes and the main preview reflects new code on next frame; Save with broken TS keeps dialog open showing compile error inline; closing dialog then Ctrl+Z removes the generated clip from timeline; Ctrl+Y restores it. "Ask AI to refactor" intentionally deferred to a follow-up feat.
...
1. **feat-009 — Action/state system**: keyframe-bind GeneratedClip params via the existing AnimationEngine + updateClipKeyframes. This was scope-cut from feat-007d.
2. **"Ask AI to refactor"** follow-up to feat-008: selection-aware prompt in SourceEditorDialog → AI returns replacement code → executeEdits over the selection. Needs a new prompt template plus streaming UI.
...
- `feature_list.json` — modified — feat-007 + feat-008 marked done with detailed evidence
- `progress.md` — modified — this file
- `apps/web/package.json` — modified — added @monaco-editor/react + monaco-editor (feat-008)
- `apps/web/src/components/SourceEditor/SourceEditorDialog.tsx` — new — Radix Dialog wrapping @monaco-editor/react Editor; Save→updateGeneratedClipSource→ensure→awaitReady; compile errors stay inline (feat-008)
- `apps/web/src/components/SourceEditor/setupMonacoEnv.ts` — new — side-effect module registering self.MonacoEnvironment.getWorker for ?worker imports of editor.worker + ts.worker (feat-008)
- `apps/web/src/components/SourceEditor/sceneDescriptionTypes.ts` — new — hand-mirrored .d.ts string of SceneDescription + Shape + GeneratedObject for Monaco intellisense via addExtraLib (feat-008)
- `apps/web/src/components/SourceEditor/index.ts` — new — barrel (feat-008)
- `apps/web/src/objects/SceneDescription.ts` — modified — added sync-warning comment pointing to sceneDescriptionTypes.ts (feat-008)
- `apps/web/src/objects/SandboxRegistry.ts` — modified — RegistryEntry.initPromise field + awaitReady(clipId, timeoutMs) method (feat-008)
- `apps/web/src/objects/SandboxRegistry.test.ts` — modified — +4 tests covering awaitReady success / compile-error / unregistered-clip / source-change paths (feat-008)
- `apps/web/src/stores/ui-store.ts` — modified — added sourceEditorClipId + openSourceEditor + closeSourceEditor (feat-008)
- `apps/web/src/App.tsx` — modified — React.lazy import of SourceEditorDialog, mounted inside Suspense fallback={null} next to AIPanel (feat-008)
- `apps/web/src/components/editor/timeline/ShapeClipComponent.tsx` — modified — handleDoubleClick fires only for clip.type === 'generated', bails when isTrimming/isDragging (feat-008)
- `apps/web/src/services/keyboard-shortcuts.ts` — modified — widened input-target whitelist to also bail on isContentEditable + [role='dialog'] + .monaco-editor (feat-008 fix; otherwise Space / Ctrl+C/V / Ctrl+Z typed in Monaco get hijacked into playPause / copyClip / projectUndo)
- `apps/web/src/stores/project-store.ts` — modified — undo/redo dispatch grew 'generated' branch; undo path calls SandboxRegistry.dispose before deleteGeneratedClip, redo recreates from snapshot via createGenerated (feat-008 fix for feat-007 omission)
